...
def test_prepare_ncep_reanalysis1_pr_wtr(tmpdir):
    ...
    expected_doc = [
        {
            ...
            'geometry': {
                'coordinates': [[[-1.25, 91.25], [-1.25, -91.25], [358.75, -91.25], [358.75, 91.25], [-1.25, 91.25]]],
                'type': 'Polygon'
            },
            'grids': {
                'default': {
    ...
    for idx in range(len(expected_doc)):
        doc_diff = _diff(expected_doc[idx], docs[idx], exclude_paths="root['properties']['odc:creation_datetime']")
